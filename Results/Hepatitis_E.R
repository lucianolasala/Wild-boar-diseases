...
occs <- read.csv("C:/Users/User/Documents/Analyses/Wild boar diseases/Hepatitis E/Hepatitis_E.csv", sep = ",", dec = ".") %>%
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>%
  st_join(arg)

occs1 <- st_as_sf(occs, coords = c("Long","Lat"), crs = 4326)

clusters <- st_read("C:/Users/User/Documents/Analyses/Wild boar diseases/Hepatitis E/Output/Hepatitis_E.col.shp")

AD <- ggplot() +
  geom_sf(data = studyarea, color = "black", alpha = 1, aes(fill = NAME_2), show.legend = FALSE) +
  geom_sf(data = clusters, alpha = 0, color = "red", size = 1) +
  geom_sf(data = occs, alpha = 1, pch = 21, size = 3, show.legend = FALSE, aes(fill = factor(Number))) +
  scale_fill_manual(values = c("#FFFFFF","#2ECC71","#F5CD60","#ABB2B9","#F0B27A")) +
  ...
  annotate("text", x = -64.0, y = -40.6, label = "RR = 12", color = "black", size = 3.5, fontface = 2)
...
ggsave(filename = "C:/Users/User/Documents/Analyses/Wild boar diseases/R_project/Wild-boar-diseases/Maps/Hepatitis_ E_clusters.jpg", plot = plot.with.inset, device = "jpeg", path = NULL,
