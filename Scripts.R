...
occs <- read.csv("C:/Users/User/Documents/Analyses/Wild boar diseases/Aujeszky/Output/Aujeszky.csv", sep = ",", dec = ".") %>%
...
###################################################################################
# Since the cluster was only nearly significant, we perform a distance analysis

# Pig farms. Las granjas son procesadas creando un raster de distancia entre cada pixel
# y la granja m?s cercana

rm(list=ls(all=TRUE))

farms <- readOGR("C:/Users/User/Documents/Analyses/Wild boar ENM/Spatial data/Capas SENASA/Farm distribution.shp")

area_estudio <- st_read("C:/Users/User/Documents/Analyses/Wild boar diseases/Shapefiles/Study_area/Study_area.shp")

area_estudio <- cbind(area_estudio, st_coordinates(st_centroid(area_estudio)))
head(area_estudio)
...
farms <- st_as_sf(fall.within.poly, coords = c("Longitud","Latitud"), crs = 4326)
farms

farms1 <- cbind(farms, st_coordinates(st_centroid(farms)))
head(farms1)

# Subset positive boars

points <- read.csv("C:/Users/User/Documents/Analyses/Wild boar diseases/Aujeszky/Output/Aujeszky.csv", sep = ",")
points
...
farms <- ggplot() +
  geom_sf(data = area_estudio, color = "black", alpha = 1, aes(fill = NAME_2), show.legend = FALSE) +
  scale_fill_manual(values = c("#F4D03F","#58D68D","#95A5A6")) +  
  geom_sf(data = farms1, alpha = 1, pch = 21, size = 2, color = "black") + 
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank()) +
  coord_sf(xlim = c(-65.5, -62), ylim = c(-41.25, -38.5)) +
  geom_point(data = posits, aes(x = Long, y = Lat), color = "red")
...
occs <- st_as_sf(points, coords = c("Long","Lat"), crs = 4326)
occs

occs1 <- cbind(occs, st_coordinates(st_centroid(occs)))
head(occs1)

occs1$Resultado <- as.factor(occs1$Resultado)
is.factor(occs1$Resultado)
...
#occs$Resultado <- as.factor(occs$Resultado)
...
#geom_sf(data = clusters, color = "red", alpha = 0)   
#panel.border = element_rect(colour = "black", fill = NA, size = 1),
#geom_rect(aes(xmin =, ymin =, xmax = ,ymax = ))
#geom_sf(data = sites, size = 3, shape = 0, color = "red")
##geom_sf(data = occs, alpha = 1, pch = 21, size = 2.5, aes(fill = factor(Resultado)), show.legend = FALSE) +

#######################

# Finding the nearest distance between two SpatialPointsDataframes 
# using gDistance rgeos? (https://stackoverflow.com/questions/42237021/finding-the-nearest-distance-between-two-spatialpointsdataframes-using-gdistance)
